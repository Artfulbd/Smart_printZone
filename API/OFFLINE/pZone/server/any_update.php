<?php
    ...
    if($link == null){
        ...
        // abort code
        $has_abort_qry = "SELECT num as 'index', name, id, printer_name FROM `prin23422ting_queue21314` pq join _user711qd9m usr on pq.u_id = usr.id join print43er_details234c23452 pd on pd.printer_id = pq.p_id where pq.abort = 1";
        ...
        // queue size
        $max_queue_qry = "SELECT `value` FROM `creadentials` WHERE type = 'max_queue_size'";
        $res = mysqli_fetch_all(mysqli_query($link, $max_queue_qry), MYSQLI_ASSOC);
        $max_queue_size = $res[0]['value'];
        ...
        join print43er_details234c23452 pd on pd.printer_id = pq.p_id ORDER BY pq.num ASC";
        ...
        if(sizeof($punch_data) < $max_queue_size)
        {
            ...
            WHERE pd.current_status = 1 ORDER by `required_time` ASC LIMIT 1";
